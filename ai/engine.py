...
def generar_insight_tarea(t: dict) -> str:
    ...
    is_completed = t.get('completada') == 1 or bool(t.get('completada'))

    if is_completed:
        prompt = (
            "Actúa como un Asesor Ejecutivo de Estrategia y Control de Proyectos para el Director General (Titular de la Dependencia).\n"
            "Debes analizar la siguiente tarea estratégica (EE) que ya ha sido COMPLETADA y redactar un reporte analítico de cierre sumamente directo para el Jefe.\n\n"
            f"DATOS DE LA TAREA COMPLETADA:\n"
            f"- Nombre de la Tarea: {t['nombre_tarea']}\n"
            f"- Proyecto Origen: {t['proyecto_origen']}\n"
            f"- Equipo: {t['equipo']}\n"
            f"- Responsable Asignado: {t['asignado'] or 'Sin Asignar'}\n"
            f"- Estado de Entrega: Completada\n"
            f"- Fecha de Vencimiento original: {t['fecha_vencimiento'] or 'No especificada'}\n"
            f"- Porcentaje de Avance declarado: {t['avance'] or '100%'}\n"
            f"- Descripción Registrada: {desc}\n"
            f"- Comentarios de Seguimiento y Cierre registrados:\n{ultimos_comentarios}\n\n"
            "REGLAS E INSTRUCCIONES PARA EL ANÁLISIS DE CIERRE:\n"
            "1. Estructura tu respuesta exactamente con estas 3 secciones usando los siguientes títulos de markdown:\n\n"
            "### 📋 RESUMEN DE ENTREGA Y CONTEXTO\n"
            "- Explica de qué trató la tarea y cómo se resolvió basándote en la descripción y los comentarios de cierre.\n"
            "- Si no hay suficientes comentarios de cierre, señálalo: '⚠️ NOTA: El entregable se marcó como completado pero hay poco detalle registrado en los comentarios sobre el método final de solución.'\n\n"
            "### 🔍 ANÁLISIS DE LA SOLUCIÓN Y SENTIDO TÉCNICO\n"
            "- Deduce si la solución aplicada hace sentido de acuerdo al alcance original de la descripción.\n"
            "- Comenta si fue entregada dentro del plazo o si requirió reprogramaciones (si es deducible de los comentarios/fechas).\n\n"
            "### ⚡ LECCIONES APRENDIDAS Y SIGUIENTES PASOS\n"
            "- Prescribe qué debe saber el Titular ahora que la tarea está completada.\n"
            "- Detalla de forma ejecutiva los siguientes pasos (ej. dar seguimiento al entregable en producción, verificar que la contraparte recibió el documento, capitalizar la solución en otros proyectos, o archivar el expediente de este entregable).\n\n"
            "REGLAS GENERALES:\n"
            "- Sé directo, conciso y con un lenguaje de alto nivel gerencial.\n"
            "- No uses introducciones ni saludos redundantes. Empieza directamente con los títulos de markdown indicados.\n"
